...
import Parser.NewParse (Expr (..), Stage1, parseExpr)
...
expr2LispVal (Part h (PartArgs args)) =
  listArgs apply' h args
  where
    apply' e es = List $ Atom "Part" : e : es
...
expr2LispVal (Apply11 e1 e2) =
  twoArgs apply' e1 e2
  where
    apply' l1 l2 = List [Atom "Apply", l1, l2, list [integer 1]]
expr2LispVal (Derivative n e) =
  let
    n' = integer n
    deriv l = List [List [Atom "Derivative", n'], l]
  in
    oneArg deriv e
