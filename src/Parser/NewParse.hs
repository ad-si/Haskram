...
specialForms =
  let forms = map try (patternBlankEs ++ blankEs ++ reverse blks ++ reverse patternBlanks)
  in  lexeme $ foldr1 (<|>) forms
...
slots =
  let lis = [slotSeqn, slotSeq, slotn, slot]
  in  lexeme $ foldr1 (<|>) (map try lis)
