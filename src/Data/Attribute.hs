...
import Data.DataType (LispVal (Atom, List), deleteSameHead)
...
allAttr att h =
  attEvalOrderless att
    . attEvalFlatten att h
    . attEvalSeqHold att
...
attributeTransform att (List lis@(h : rest)) =
  let attrs = getAttributes h att
  in  attTransOneIdent attrs lis
