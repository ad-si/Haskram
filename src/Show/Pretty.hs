...
import Data.DataType (LispVal (Atom, List, Number), fullForm, tshow)
import Data.List ()
...
prettyPrint' _ (List (Atom "List" : xs)) =
  let args = map prettyPrint xs
  in  curlyBrack $ T.intercalate "," args
...
prettyPrint' now (List (Atom name : xs)) =
  let prec = getPrecedence name
  in  case prec of
        Nothing -> functionWrap name (map prettyPrint xs)
        Just n ->
          let args = map (prettyPrint' n) xs
              form = infixForm name
              result = addInfix form args
          in  if now <= n then bracket result else result
