...
import Data.DataType (
  IOThrowsError,
  LispError (NumArgs, NumArgsBetween, NumArgsMore),
  LispVal (List),
  unpackAtom,
 )
...
manynop n = checkArgsNumber (>= n) throw
  where
    throw val x = throwError (NumArgsMore (unpackAtom val) n x)
...
between l r = checkArgsNumber (\x -> x >= l && x <= r) throw
  where
    throw val x = throwError (NumArgsBetween (unpackAtom val) l r x)


-- | Ensure that the argument list has excatly n elements.
withnop n = checkArgsNumber (== n) throw
  where
    throw val x = throwError (NumArgs (unpackAtom val) n x)
