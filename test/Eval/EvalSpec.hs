...
import Data.Number.Number (Number (Integer))
import Data.Ratio ((%))
import Eval.Eval (eval, initialState)
import Parser.Trans (readExpr)
import System.IO.Unsafe (unsafePerformIO)
import Test (list, one, rational, readVal, three, two)
...
test a b =
  let parsed = extractValue $ mapM readExpr a
  in  testEval parsed `shouldBe` b
...
testEval exprs =
  let expr = mapM eval exprs
      evaled = runExceptT $ evalStateT expr initialState
  in  last (extractValue (unsafePerformIO evaled))
...
spec = do
  ...
  context "eval with context" $ do
    ...
    it "pattern matching" $ do
      test
        ["a[0]=1", "a[0.0]=1", "a[0]", "a[0.0]"]
        (integer 1)
    it "pattern test" $ do
      test
        ["f[a_]=a", "f[a_?(#>10&)]=10", "f[1100]"]
        (integer 10)
      test
        ["12/.x_?(#>111&)->1"]
        (integer 12)
      test3
        ["f[a_?(False&)]=100", "f[1000]"]
        "f[1000]"
      test
        [ "fib[n_]:=fib[n]=fib[n-1]+fib[n-2]"
        , "fib[n_?(#==1&)]=1"
      ...
      test
        ...
        (integer 2)
      test
        ["{1,1,2,2,3}/.{x_,x_,y_,y_,z_} -> 2"]
        (integer 2)

      test3
        ["{{1,1},{0,0},{0,2}}/.{x_,y_}/;x+y==2 -> a"]
        "{a,{0,0},a}"
      test3
        ["{{1,1},{0,0},{0,2}}/.{x_,x_}/;x+x==2 -> a"]
        "{a,{0,0},{0,2}}"
      test3
        ["Condition[Condition[f[x_],x>1],x<2]=sdf", "f[3/2]"]
        "sdf"
      test3
        ["Condition[Condition[f[x_],x>1],x<2]=sdf", "f[0]"]
        "f[0]"
      test3
        ["Condition[Condition[f[x_],x>1],x<2]=sdf", "f[2]"]
        "f[2]"
      test3
    ...
    it "sequence test" $ do
      ...
      test3
        ["f[a_*b__]:=f[a]+Sequence@@(Map[f,{b}])", "f[x y z k l]"]
        "f[k]+f[l]+f[x]+f[y]+f[z]"
      test3
        ["f[x__] := Length[{x}]", "{f[x, y, z], f[]}"]
        "{3,f[]}"
      test3
        ["f[x___] := p[x, x]", "{f[], f[1], f[1, a]}"]
        "{p[],p[1,1],p[1,a,1,a]}"
      test3
        ["f[x___]:=p[x,Plus[x]]", "{f[1], f[1,2],f[1,2,x],f[1,2,3]}"]
        "{p[1,1],p[1,2,3],p[1,2,x,3+x],p[1,2,3,6]}"
      test3
        ["f[x_,y___]:=Plus[y]^x", "{f[1,2,3], f[23,5,2], f[23,af,23,l],f[]}"]
